...
# excelファイルの内容を読み込み
df = pd.read_excel('S:/個人作業用/渡邊/ワールドジャパン/学習用データ/サロンカルテデータ_相馬さん/2フォーマット(柏葉さん)/【E】エステティックサービス契約書.xlsx', sheet_name='【E】エステティックサービス契約書', header=1)
# 不足している列を追加
df.insert(loc = 23,column = 'コース名5行目', value = None)
df.insert(loc = 24,column = 'コース名6行目', value = None)
df.insert(loc = 29,column = '時間5行目', value = None)
df.insert(loc = 30,column = '時間6行目', value = None)
df.insert(loc = 35,column = '単価5行目', value = None)
df.insert(loc = 36,column = '単価6行目', value = None)
df.insert(loc = 41,column = '回数5行目', value = None)
df.insert(loc = 42,column = '回数6行目', value = None)
df.insert(loc = 48,column = '総時間数5行目', value = None)
df.insert(loc = 49,column = '総時間数6行目', value = None)
df.insert(loc = 55,column = '金額5行目', value = None)
df.insert(loc = 56,column = '金額6行目', value = None)
df.insert(loc = 80,column = '数量6行目', value = None)

# 不要な値を含む行を削除
drop_index = df.index[df['お名前'] == ' ']
df = df.drop(drop_index)
df = df.dropna(subset=['お名前'])

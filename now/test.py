...
#学習用データの保存ディレクトリ
base_dir = 'S:/個人作業用/宇野/ワールドジャパン/学習用データ(テスト用)/2フォーマット 2/【B】'
...
for nam in files:
  ...
  for sheet in bk :
      ...
      if 'ボディ' in sheet.title:
        print(sheet.title + ', ' + str(sheet.max_row) + ', ' + str(sheet.cell(row=sheet.max_row, column=2).value))
...
for one in ary :
  shutil.move(one, one.replace('/【B】/', '/【B】/【要修正】/'))
"""
